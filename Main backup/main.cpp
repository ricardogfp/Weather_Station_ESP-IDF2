...
#include "APDS9930.h"  // Include APDS9930 header

// Tag for logging
static const char *MAIN_TAG = "Main";

// APDS9930 sensor object
APDS9930 apds(I2C_MASTER_NUM); // Pass I2C port number (0)
...
static void proximity_sensor_task(void *pvParameters); // Declare proximity sensor task
 ...
 */
 void app_main(void)
 {
    ...
    // Initialize APDS9930 Proximity Sensor with retry
    ESP_LOGI(MAIN_TAG, "Initializing APDS9930 Proximity Sensor");
    // Add delay to ensure I2C bus is stable after LCD init
    ...
    for (int retry = 0; retry < max_retries; retry++) {
        ESP_LOGI(MAIN_TAG, "APDS9930 init attempt %d of %d", retry + 1, max_retries);
        
        if (apds.init()) {
            ESP_LOGI(MAIN_TAG, "APDS9930 init success");
            sensor_initialized = true;
            
            if (apds.enableProximity(false)) { // false = do not enable proximity interrupts
                ESP_LOGI(MAIN_TAG, "APDS9930 Proximity sensor enabled");
            } else {
                ESP_LOGW(MAIN_TAG, "Failed to enable APDS9930 Proximity sensor");
            }
            break;
        } else {
            ESP_LOGW(MAIN_TAG, "APDS9930 init failed on attempt %d", retry + 1);
            vTaskDelay(pdMS_TO_TICKS(100)); // Wait before retry
        }
    }

    if (!sensor_initialized) {
        ESP_LOGE(MAIN_TAG, "APDS9930 initialization failed after %d attempts", max_retries);
    }
     ...
     struct tm default_timeinfo = {
         ...
         .tm_mon = 5 - 1,
         .tm_year = 2025 - 1900,
         .tm_wday = 0,    // Day of week (0-6, Sunday = 0) - mktime will calculate
         .tm_yday = 0,    // Day of year (0-365) - mktime will calculate
         .tm_isdst = -1   // Daylight Saving Time flag (-1 for unknown)
     };
    ...
    // Ensure default time is set if RTC is commented out
    ESP_LOGI(MAIN_TAG, "Using default time as RTC logic is bypassed for debugging.");
    tv.tv_sec = default_time;
    tv.tv_usec = 0;
    settimeofday(&tv, NULL);
    time_source = TIME_SOURCE_INTERNAL; // Explicitly set to internal
    ...
    // Create task for proximity sensor
    // xTaskCreate(proximity_sensor_task, "proximity_task", 2048, NULL, 5, NULL);
    // ESP_LOGI(MAIN_TAG, "Proximity sensor task created");
...
// Task to read proximity sensor data
static void proximity_sensor_task(void *pvParameters) {
    uint16_t proximity_data;
    while (1) {
        if (apds.readProximityValue(proximity_data)) {
            ESP_LOGI(MAIN_TAG, "Proximity: %u", proximity_data);
        } else {
            ESP_LOGE(MAIN_TAG, "Failed to read proximity data");
        }
